...
export default ({
  ...
  const woff2 = has(embed, 'woff2') ? encodeFont(config.woff2) : `"${filename}.woff2"`;
  const woff = has(embed, 'woff') ? encodeFont(config.woff) : `"${filename}.woff"`;
  const ttf = has(embed, 'ttf') ? encodeFont(config.ttf) : `"${filename}.ttf"`;
  const [svg, embedSvg] = has(embed, 'svg') ? [encodeSvg(config.svg), true] : [undefined, false];
